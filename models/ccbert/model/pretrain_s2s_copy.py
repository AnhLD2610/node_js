...
class Seq2Seq(nn.Module):
    def __init__(self, encoder_des, decoder_des, encoder_code, decoder_code, config, beam_size, max_length, sos_id, eos_id, pad_id):
        super(Seq2Seq, self).__init__()
        self.encoder_des = encoder_des
        self.decoder_des=   decoder_des
    ...
    def tie_weights(self):
        ...
        self._tie_or_clone_weights(self.lm_head,
                                   self.encoder_des.embeddings.word_embeddings)        
        
    def forward(self, des_ids=None,des_mask=None, code_ids=None, code_mask=None, target_ids=None,target_mask=None):   
        '''
        ...
        '''
        outputs_des = self.encoder_des(des_ids, attention_mask=des_mask) # output[0] [batch_size, max_src_len, hidden_dim]
        encoder_output_des = outputs_des[0].permute([1,0,2]).contiguous() # [max_src_len, batch_size, hidden_dim]
        
        outputs_code = self.encoder_code(code_ids, attention_mask=code_mask) # output[0] [batch_size, max_src_len, hidden_dim]
        encoder_output_code = outputs_code[0].permute([1,0,2]).contiguous() # [max_src_len, batch_size, hidden_dim]
        if target_ids is not None:  
            ...
            tgt_embeddings_des = self.encoder_des.embeddings(target_ids).permute([1,0,2]).contiguous() # [max_tgt_len, batch_size, hidden_dim]
            out_des = self.decoder_des(tgt_embeddings_des,encoder_output_des, tgt_mask=attn_mask, memory_key_padding_mask=(1-des_mask).bool()) # [max_tgt_len, batch_size, hidden_dim]
            hidden_states = torch.tanh(self.dense(out_des)).permute([1,0,2]).contiguous() # [batch_size, max_tgt_len, hidden_dim]
            # final_prob_des [tgt_len, batch_size, vocab_size]
            final_prob_des = self.copy_attention(encoder_output_des, hidden_states.transpose(0, 1),
                                                des_ids.transpose(0, 1), tgt_embeddings_des)
            final_prob_des = final_prob_des.transpose(0, 1) # [batch_size, tgt_len, vocab_size]

            
            tgt_embeddings_code = self.encoder_code.embeddings(target_ids).permute([1,0,2]).contiguous() # [max_tgt_len, batch_size, hidden_dim]
            out_code = self.decoder_code(tgt_embeddings_code,encoder_output_code, tgt_mask=attn_mask, memory_key_padding_mask=(1-code_mask).bool()) # [max_tgt_len, batch_size, hidden_dim]
            hidden_states = torch.tanh(self.dense(out_code)).permute([1,0,2]).contiguous() # [batch_size, max_tgt_len, hidden_dim]
            # final_prob_code [tgt_len, batch_size, vocab_size]
            final_prob_code = self.copy_attention(encoder_output_code, hidden_states.transpose(0, 1),
                                                code_ids.transpose(0, 1), tgt_embeddings_code)            
            ...
            loss = loss_fct(shift_logits.reshape(-1, shift_logits.size(-1))[active_loss],
                            shift_labels.reshape(-1)[active_loss])
        else:
            #Predict 
            preds=[]           
            zero=torch.cuda.LongTensor(1).fill_(0) # define a tensor valued 0
            for i in range(des_ids.shape[0]): # loop for batch_size times
                context_des = encoder_output_des[:, i:i+1]  # [max_src_len_des, 1, hidden_dim]
                context_mask_des=des_mask[i:i+1,:]
                context_code = encoder_output_code[:, i:i+1]  # [max_src_len_code, 1, hidden_dim]
                context_mask_code=code_mask[i:i+1,:]
                # Concatenate along the first axis (max_src_len)
                beam = Beam(self.beam_size,self.sos_id,self.eos_id)
                input_ids=beam.getCurrentState().to(device) # [beam_size, tgt_len]
                context_des=context_des.repeat(1, self.beam_size,1) # [max_src_len, beam_size, hidden_dim]
                context_mask_des=context_mask_des.repeat(self.beam_size,1) # [beam_size, max_src_len, hidden_dim]
                context_code=context_code.repeat(1, self.beam_size,1) # [max_src_len, beam_size, hidden_dim]
                context_mask_code=context_mask_code.repeat(self.beam_size,1) # [beam_size, max_src_len, hidden_dim]
                for _ in range(self.max_length): # loop for max_tgt_len times
                    ...
                    tgt_embeddings_des = self.encoder_des.embeddings(input_ids).permute([1,0,2]).contiguous() # [tgt_len, beam_size, hidden_dim]
                    out_des = self.decoder_des(tgt_embeddings_des,context_des,tgt_mask=attn_mask,memory_key_padding_mask=(1-context_mask_des).bool()) # [tgt_len, beam_size, hidden_dim]
                    out_des = torch.tanh(self.dense(out_des))  # [tgt_len, beam_size, hidden_dim]
                    hidden_states=out_des.permute([1,0,2]).contiguous()[:,-1,:] # [beam_size, hidden_dim]
                    decoder_state = hidden_states.unsqueeze(0) # [1, beam_size, hidden_dim]
                    beam_des_ids = des_ids[i].unsqueeze(0) # [1, max_src_len]
                    beam_des_ids = beam_des_ids.repeat(self.beam_size, 1) # [beam_size, max_src_len]
                    # final_prob [1, beam_size, vocab_size]
                    final_prob_des = self.copy_attention(context_des, decoder_state,
                                                beam_des_ids.transpose(0, 1), tgt_embeddings_des[-1].unsqueeze(0))
                    final_prob_des = final_prob_des.squeeze(0) # [beam_size, vocab_size]
                    
                    
                    tgt_embeddings_code = self.encoder_code.embeddings(input_ids).permute([1,0,2]).contiguous() # [tgt_len, beam_size, hidden_dim]
                    out_code = self.decoder_code(tgt_embeddings_code,context_code,tgt_mask=attn_mask,memory_key_padding_mask=(1-context_mask_code).bool()) # [tgt_len, beam_size, hidden_dim]
                    out_code = torch.tanh(self.dense(out_code))  # [tgt_len, beam_size, hidden_dim]
                    hidden_states=out_code.permute([1,0,2]).contiguous()[:,-1,:] # [beam_size, hidden_dim]
                    decoder_state = hidden_states.unsqueeze(0) # [1, beam_size, hidden_dim]
                    beam_code_ids = code_ids[i].unsqueeze(0) # [1, max_src_len]
                    beam_code_ids = beam_code_ids.repeat(self.beam_size, 1) # [beam_size, max_src_len]
                    # final_prob [1, beam_size, vocab_size]
                    final_prob_code = self.copy_attention(context_code, decoder_state,
                                                beam_code_ids.transpose(0, 1), tgt_embeddings_code[-1].unsqueeze(0))
                    final_prob_code = final_prob_code.squeeze(0) # [beam_size, vocab_size]
                ...
                pred=[torch.cat([x.reshape(-1) for x in p]+[zero]*(self.max_length-len(p))).reshape(1,-1) for p in pred] # [beam_size, 1, max_tgt_len]
...
class Beam(object):
    ...
    def getCurrentState(self):
        ...
        batch = self.tt.LongTensor(self.nextYs[-1]).reshape(-1, 1)
        return batch
