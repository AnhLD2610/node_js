...
import ipdb
import math
from transformers import RobertaConfig, RobertaModel, AutoModel, AdamW, get_linear_schedule_with_warmup
...
#     print(batch[0])
#     print(batch[0].shape)
#     sen = batch[0].reshape(8,512)
#     # batch[0] shape = [512,8]
#     xem lop decoder ho lam nhu nao
#     # Convert tensor elements back to sentences
# for i, batch in enumerate(train_dataloader):
#     batch = tuple(t.to(device) for t in batch)
#     # source_ids [seq_len, batch_size]
#     source_ids, source_mask, target_ids, target_mask = batch
#     source_ids_T = source_ids.transpose(0, 1) # [batch_size, src_len] for multi 
#     print(source_ids_T.shape)
#     target_input_T = target_ids.transpose(0, 1)
#     print(target_input_T.shape)
#     # print(target_input_T)
#     source_sentences = decode_batch_ids(source_ids_T[:,1:])

#     target_sentences = decode_batch_ids(target_input_T[:, 1:])
...
#     model.to(device)
#     context_embeddings = model(source_ids_T[0][None, :])[0]
#     for i in range(1,source_ids_T.shape[0]):
#         sub_tensor = source_ids_T[i]  # Get the sub-tensor with shape [512]
#         context_embeddings_new = model(sub_tensor[None, :])[0]
#         context_embeddings = torch.cat([context_embeddings, context_embeddings_new], dim=0)
...
model = Seq2Seq(encoder, decoder, encoder, decoder, encoder_config, args.beam_size, args.max_tgt_len,
...
# # total_params_en = count_parameters(encoder)
# total_params_de = count_parameters(decoder)


# print(f"Total Parameters in Sample Model: {total_params}")
# # print(f"Total Parameters in Sample Model: {total_params_en}")
# print(f"Total Parameters in Sample Model: {total_params_de}")
...
def get_one_example(target_ids, pred_dist):
...
def train(model, iterator, optimizer, best_rouge):
    ...
    for i, batch in enumerate(iterator):
        batch = tuple(t.to(device) for t in batch)
        # source_ids [seq_len, batch_size]
        des_ids, des_mask ,code_ids, code_mask, target_ids, target_mask = batch
        des_ids_T = des_ids.transpose(0, 1) # [batch_size, src_len] for multi gpu
        code_ids_T = code_ids.transpose(0, 1)
        target_input_T = target_ids.transpose(0, 1)
        
        des_mask_T = des_mask.transpose(0, 1)
        code_mask_T = code_mask.transpose(0, 1)
        target_mask_T = target_mask.transpose(0, 1)
        
        loss, logits = model(des_ids_T, des_mask_T, code_ids_T, code_mask_T, target_input_T, target_mask_T) # [batch_size, seq_len]
        ...
        if args.debug:
            (predict, target) = get_one_example(target_ids, logits)
            ipdb.set_trace()
...
def evaluate(model, iterator, best_rouge):
    ...
    with torch.no_grad():
        for i, batch in enumerate(iterator):
            batch = tuple(t.to(device) for t in batch)
            # source_ids [seq_len, batch_size]
            des_ids, des_mask, code_ids, code_mask, target_ids, _ = batch
            # logits [batch_size, tgt_len, out_dim]
            des_ids_T = des_ids.transpose(0, 1) # [batch_size, src_len] for multi gpu
            des_mask_T = des_mask.transpose(0, 1)
            code_ids_T = code_ids.transpose(0, 1) # [batch_size, src_len] for multi gpu
            code_mask_T = code_mask.transpose(0, 1)
            _, logits = model(des_ids_T, des_mask_T, code_ids_T, code_mask_T) # [batch_size, seq_len]
